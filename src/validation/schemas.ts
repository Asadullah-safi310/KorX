...
export const propertyBaseSchema = {
  ...
  area_size: Yup.number().typeError('Must be a number').required('Area size is required').positive(),
  bedrooms: Yup.number().when('property_type', {
    is: (val: string) => val && val.toLowerCase() !== 'plot',
    then: (schema) => schema.required('Bedrooms required').min(0),
    otherwise: (schema) => schema.nullable(),
  }),
  bathrooms: Yup.number().when('property_type', {
    is: (val: string) => val && val.toLowerCase() !== 'plot',
  ...
  sale_price: Yup.number().when('is_available_for_sale', {
    is: true,
    then: (schema) => schema.required('Sale price is required').positive(),
    otherwise: (schema) => schema.nullable(),
  }),
  rent_price: Yup.number().when('is_available_for_rent', {
    is: true,
